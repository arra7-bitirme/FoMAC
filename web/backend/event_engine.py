...
DEBOUNCE_FRAMES: Dict[str, int] = {
    "possession_change": 75,   # top el değiştirme — 1.5s @ 50fps; tek dokunuş spam önle
    "zone_change":       30,   # bölge değişimi
    "sprint":           150,   # depar — 3 saniye @ 50 fps (agresif bastırma)
    "loose_ball":       100,   # top serbest — 2s @ 50fps; geçici kayıplarda spam önle
    "danger":             5,   # tehlike — mümkün olduğunca sık bildir
    "pass":              10,   # pas tespiti
    "cross":             10,   # orta tespiti
    "ball_carry":        75,   # uzun top taşıma olayı
}
...
class EventEngine:
    ...
    def __init__(
        ...
        # ── Top Taşıma Süresi Takibi ─────────────────────────────────────────
        # track_id → topla geçirilen frame sayısı (arka arkaya)
        self._possession_frames: Dict[int, int] = {}
    ...
    def _generate_events(
        ...
        # ── Top taşıma sayacını güncelle ────────────────────────────────────
        if curr_poss is not None:
            self._possession_frames[curr_poss] = self._possession_frames.get(curr_poss, 0) + 1
        if possession_changed and prev_poss is not None:
            self._possession_frames.pop(prev_poss, None)
        ...
        if possession_changed:
            if curr_poss is not None and curr_poss in curr.players:
                ...
                if ps.is_sprinting and self._debounce_ok("possession_change", curr_poss, frame_id):
                    ...
                    if ps.zone == PitchZone.OPPONENT_PENALTY:
                        priority = PRIORITY_HIGH
                        msg = f"Top {label}'da! Rakip ceza sahasında tehlikeli pozisyon."
                    elif ps.zone == PitchZone.OPPONENT_HALF:
                        priority = PRIORITY_MID
                        action = random.choice([
                            "rakip yarısında topu kontrol altına aldı",
                            "rakip sahasında ilerliyor",
                            "topla rakip kalesine doğru yöneliyor",
                        ])
                        msg = f"{label} {action}."
                    elif ps.zone == PitchZone.CENTER_CIRCLE:
                        priority = PRIORITY_LOW
                        msg = f"Top {label}'da, orta sahada oyun kuruluyor."
                    else:
                        priority = PRIORITY_LOW
                        msg = f"{label} topu aldı, {zone_name} bölgesinden hücum başlatmaya çalışıyor."
                    events.append(Event(
                        frame_id=frame_id, priority=priority,
                        event_type="possession_change", message=msg,
                        track_id=curr_poss, team_id=ps.team_id,
        ...
        # ── 1b. Uzun Top Taşıma (ball_carry) ────────────────────────────────
        # Aynı oyuncu 3+ saniye topla ilerliyorsa bildir (possession_change'den bağımsız)
        CARRY_MIN_FRAMES = int(self.fps * 3.0)
        if (curr_poss is not None and curr_poss in curr.players
                and not possession_changed):
            carry_frames = self._possession_frames.get(curr_poss, 0)
            if carry_frames >= CARRY_MIN_FRAMES and carry_frames % CARRY_MIN_FRAMES == 0:
                ps = curr.players[curr_poss]
                label = self._player_label(ps)
                if self._debounce_ok("ball_carry", curr_poss, frame_id):
                    if ps.zone == PitchZone.OPPONENT_PENALTY:
                        priority = PRIORITY_HIGH
                        msg = f"{label} uzun süredir ceza sahasında topla, şut arayışında!"
                    elif ps.zone == PitchZone.OPPONENT_HALF:
                        priority = PRIORITY_MID
                        msg = f"{label} rakip yarı sahada atağı sürdürüyor."
                    else:
                        priority = PRIORITY_LOW
                        msg = f"{label} {self._speed_description(ps.speed_mps)}, topu elinde tutuyor."
                    events.append(Event(
                        frame_id=frame_id, priority=priority,
                        event_type="ball_carry", message=msg,
                        track_id=curr_poss, team_id=ps.team_id,
                    ))
