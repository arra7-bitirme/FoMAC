"""
Ball Dataset Extraction Module

Handles extraction of ball detection dataset from Hugging Face format
and conversion to YOLO format for training.
"""
...
class DatasetExtractor:
    """Extracts and converts ball detection dataset to YOLO format."""
    ...
    def extract_dataset(self) -> Path:
        """
        Extract and convert ball detection dataset to YOLO format.
        ...
        logger.info("=" * 80)
        logger.info("BALL DETECTION DATASET EXTRACTION")
        logger.info("=" * 80)
        
        ballDataset_path = self._get_ball_dataset_path()
        output_root = self._get_output_root()
        
        logger.info(f"Source: {ballDataset_path}")
        logger.info(f"Output: {output_root}")
        
        self._convert_huggingface_to_yolo(ballDataset_path, output_root)
    ...
    def _convert_huggingface_to_yolo(self, ballDataset_path: Path, output_root: Path):
        ...
        try:
            ...
            for i in tqdm(range(train_split), desc="Converting training data"):
                ...
                self._process_sample(sample, i, train_images_dir, train_labels_dir)
            
            # Process validation samples
            for i in tqdm(range(train_split, total_samples), desc="Converting validation data"):
                sample = train_dataset[i]
                self._process_sample(sample, i, val_images_dir, val_labels_dir)
                
        except Exception as e:
            logger.error(f"Error converting dataset: {e}")
            # Fallback: copy existing structure if available
            self._copy_existing_dataset_structure(ballDataset_path, output_root)
    
    def _process_sample(self, sample: Dict[str, Any], idx: int, images_dir: Path, labels_dir: Path):
        """Process a single sample from the dataset."""
        try:
            # Save image
            image = sample['image']
            if isinstance(image, Image.Image):
                image_path = images_dir / f"image_{idx:06d}.jpg"
                image.save(image_path, quality=95)
                
                # Create corresponding label file (assuming ball detection)
                # For now, create empty label file - you may need to modify this
                # based on your actual dataset structure
                label_path = labels_dir / f"image_{idx:06d}.txt"
                
                # If your dataset has bounding box annotations, process them here
                # For now, creating empty files
                with open(label_path, 'w') as f:
                    # Ball class = 0, format: class x_center y_center width height (normalized)
                    # You'll need to extract actual annotations from your dataset
                    pass
                    
        except Exception as e:
            logger.warning(f"Error processing sample {idx}: {e}")
    
    def _copy_existing_dataset_structure(self, ballDataset_path: Path, output_root: Path):
        """Fallback: copy existing dataset structure if conversion fails."""
        logger.info("Attempting to use existing dataset structure...")
        
        # Check if there's already a processed structure
        potential_dirs = [
            ballDataset_path.parent / "train",
            ballDataset_path.parent / "val",
            ballDataset_path.parent / "images",
            ballDataset_path.parent / "labels"
        ]
        
        for dir_path in potential_dirs:
            if dir_path.exists():
                logger.info(f"Copying from existing structure: {dir_path}")
                dest_path = output_root / dir_path.name
                if dest_path.exists():
                    shutil.rmtree(dest_path)
                shutil.copytree(dir_path, dest_path)
                break
        else:
            logger.warning("No existing dataset structure found. You may need to manually prepare the dataset.")
