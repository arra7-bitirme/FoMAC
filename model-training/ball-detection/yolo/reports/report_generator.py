...
class TrainingReportGenerator:
    ...
    def _extract_report_data(
        ...
        # Extract YOLO training results if available
...
class EnhancedReportGenerator:
    """Enhanced report generator with baseline comparison capabilities."""
    ...
    def __init__(self, workspace_root: str, project_name: str):
        ...
        Args:
            ...
            project_name: Name of the project for report organization
        """
        self.workspace_root = Path(workspace_root)
        self.project_name = project_name
        
        # Create reports directory structure
        self.reports_root = self.workspace_root / "model-training" / "ball-detection" / "yolo" / "reports"
    ...
    def generate_training_reports(
        ...
        """
        Generate enhanced training reports with baseline comparison.
        ...
        """
        logger.info("🔄 Generating enhanced training reports...")
        
        # Create timestamp-based directory
        timestamp = datetime.now().strftime('%H%M-%d%m%Y')
        report_dir = self.reports_root / self.project_name / timestamp
        report_dir.mkdir(parents=True, exist_ok=True)
        
        # Enhance training data with baseline comparison
        enhanced_data = training_data.copy()
        if baseline_metrics:
            enhanced_data['config']['baseline_metrics'] = baseline_metrics
        if final_metrics:
            enhanced_data['final_metrics'] = final_metrics
            
        # Use the existing TrainingReportGenerator
        generator = TrainingReportGenerator(report_dir)
        
        # Generate reports using the enhanced data
        reports = generator.generate_reports(
            training_results=None,  # We already have processed data
            config=enhanced_data['config'],
            model_path=Path(enhanced_data['model_path']) if enhanced_data.get('model_path') else None,
            dataset_stats=enhanced_data.get('dataset_stats', {})
        )
        
        # Extract paths
        excel_path = str(reports['excel'])
        html_path = str(reports['html'])
        
        logger.info(f"✅ Enhanced Excel report generated: {excel_path}")
        logger.info(f"✅ Enhanced HTML report generated: {html_path}")
